...
static enumResult ExecuteMatCo(const Graph* query_graph, const Graph* data_graph, const std::vector<ui>& match_order);
...
private:
    struct MatCoContext {
        ...
        std::vector<std::vector<bool>>& candidate_set_flags;
    ...
    static void FindMatCo(ui depth, std::vector<ui>& current_match, MatCoContext& context);
    static bool computeCandidates(ui depth, const std::vector<ui>& current_match, MatCoContext& context);
