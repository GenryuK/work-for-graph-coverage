...
void GenerateQueryPlan::generateDEGQueryPlan(const Graph *data_graph, const Graph *query_graph, ui *candidates_count, ui *&order, ui *&pivot){
    const ui num_vertices = query_graph->getVerticesCount();
    std::vector<bool> visited_vertices(num_vertices, false);
    order = new ui[num_vertices];
    pivot = new ui[num_vertices];

    ui max_degree = 0;
    VertexID start_vertex = 0;
    for (ui i = 0; i< num_vertices; ++i){
        ui current_degree = query_graph->getVertexDegree(i);
    ...
    for (ui i = 1; i < num_vertices; ++i){
        VertexID best_next_vertex = 0;
        int max_adjacent_count = -1;

        for (ui j = 0; j < num_vertices; ++j){
            if (visited_vertices[j]){
                continue;
            }
            ui current_adjacent_count = 0;
            ui neighbor_count;
            const ui* neighbors = query_graph->getVertexNeighbors(j, neighbor_count);
            for (ui k = 0; k < neighbor_count; ++k){
                if (visited_vertices[neighbors[k]]){
                    current_adjacent_count++;
                }
            }
            if (max_adjacent_count == -1 || current_adjacent_count > max_adjacent_count ||
                (current_adjacent_count == max_adjacent_count && query_graph->getVertexDegree(j) > query_graph->getVertexDegree(best_next_vertex))){
                    max_adjacent_count = current_adjacent_count;
                    best_next_vertex = j;
                }
            }
            order[i] = best_next_vertex;
            visited_vertices[best_next_vertex] = true;
    }
...
void GenerateQueryPlan::generateDSQLQueryPlan(const Graph *data_graph, const Graph *query_graph, ui *candidates_count,
                                             ui *&order, ui *&pivot) {
     ...
     VertexID start_vertex = selectDSQLStartVertex(query_graph, candidates_count);
     order[0] = start_vertex;
     updateValidVertices(query_graph, start_vertex, visited_vertices, adjacent_vertices);
     ...
     for (ui i = 1; i < query_graph->getVerticesCount(); ++i) {
          ...
          for (ui j = 0; j < query_graph->getVerticesCount(); ++j) {
               ...
               if (!visited_vertices[cur_vertex] && adjacent_vertices[cur_vertex]) {
                    if (candidates_count[cur_vertex]/query_graph->getVertexDegree(cur_vertex) < min_value) {
                         min_value = candidates_count[cur_vertex];
                         next_vertex = cur_vertex;
                    }
               }
...
VertexID GenerateQueryPlan::selectDSQLStartVertex(const Graph *query_graph, ui *candidates_count) {
     ...
     for (ui i = 1; i < query_graph->getVerticesCount(); ++i) {
          ...
          if (candidates_count[cur_vertex]/query_graph->getVertexDegree(cur_vertex) < candidates_count[start_vertex]/query_graph->getVertexDegree(start_vertex)) {
               start_vertex = cur_vertex;
          }
     }
