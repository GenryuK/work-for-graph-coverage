...
#define printingM //comment this to disable printing messages
...
int main(int argc, char** argv) {
    ...
    } else if (input_filter_type == "NON") { // これはMatCo用のfiltering technique, TSOと入れ替えている
        FilterVertices::LabelFilter(data_graph, query_graph, candidates, candidates_count);
    ...
    } else if (input_order_type == "DEG") { // MatCo用の次数しか見ないordering technique
        GenerateQueryPlan::generateDEGQueryPlan(data_graph, query_graph, candidates_count, matching_order, pivots);
...
if (enable_symmetry) {
    ...
    if (input_engine_type == "EXPLORE") {       
        ...
        //                                                   output_limit, call_count,TimeL1,FairT, ordered_constraints);
        // sには{size_t embedding_cnt; ui candidate_true_count_sum; ui Can_embed; int topk[8]; vector<set<ui>> candidate_true;}が入っている
    ...
    printf("coverage result: %zu\n", s.Can_embed);
    std::cout << "End.." << std::endl;

    std::string filename_with_ext;
    size_t last_slash_pos = input_query_graph_file.find_last_of('/');
    if (last_slash_pos != std::string::npos) {
        filename_with_ext = input_query_graph_file.substr(last_slash_pos + 1);
    } else {
        filename_with_ext = input_query_graph_file;
    }
    
    std::string filename_only;
    size_t last_dot_pos = filename_with_ext.find_last_of('.');
    if (last_dot_pos != std::string::npos) {
        filename_only = filename_with_ext.substr(0, last_dot_pos);
    } else {
        filename_only = filename_with_ext;
    }

    std::string data_with_ext;
    size_t last_slash_pos_data = input_data_graph_file.find_last_of('/');
    if (last_slash_pos_data != std::string::npos) {
        data_with_ext = input_data_graph_file.substr(last_slash_pos_data + 1);
    } else {
        data_with_ext = input_data_graph_file;
    }

    std::string data_only;
    size_t last_dot_pos_data = data_with_ext.find_last_of('.');
    if (last_dot_pos_data != std::string::npos) {
        data_only = data_with_ext.substr(0, last_dot_pos_data);
    } else {
        data_only = data_with_ext;
    }

    
    // output to csv file
    //std::ofstream ofs("/home/kuraya/MandM/Mix-Match/SubgraphMatchingSurvey-main/vlabel/output/memo.csv", std::ios::app);
    std::ofstream ofs(output_file_path, std::ios::app);
    ofs << data_only << ","
        << filename_only << ","
        << input_filter_type << ","
        << input_order_type << ","
        ...
        << preprocessing_time_in_ns * pow(10, -6) << ","
        << total_time_in_ns * pow(10, -6) << ","
