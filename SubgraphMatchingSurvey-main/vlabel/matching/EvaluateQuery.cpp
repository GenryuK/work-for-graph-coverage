...
enumResult
EvaluateQuery::DIVSMNN(const Graph *data_graph, const Graph *query_graph,ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                       ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    //assumming that creating matrix can be done before we do not count the time.
    int qsiz = query_graph->getVerticesCount();
    ...
    while (true)
    {
        ...
        while (idx[cur_depth] < idx_count[cur_depth])
        {
            ...
            // else if (candidate == 0 &&cur_depth != max_depth - 1)
            else if (candidate == 0 && cur_depth != max_depth - 1&&false)
            {
                //calculateCell(query_graph, edge_matrix, candidates_count, candidatesHC, idToValues, count2, u, valid_idx);
                calculateCell(query_graph, edge_matrix, candidates_count, candidatesHC, idToValues, count2, u, valid_idx);
                //calculateCellFN(query_graph, edge_matrix, candidates_count, candidatesHC, idToValues, count2, u, valid_idx, RQ);
...
enumResult
EvaluateQuery::DIVSMFE(const Graph *data_graph, const Graph *query_graph,ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                       ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    //assumming that creating matrix can be done before we do not count the time.
    int qsiz = query_graph->getVerticesCount();
...
EvaluateQuery::MatCo(const Graph *query_graph, const Graph *data_graph, ui **candidates, ui *candidates_count, ui *order, size_t output_limit_num){
    ...
    // depthは何番目のクエリ頂点のマッチを行っているか
    std::vector<std::vector<bool>> candidate_set_flags; // MatCoと同じ
    ...
    candidate_sets.reserve(qsize);

    for (int i = 0; i < qsize; ++i){
        // std::cout<<"1.i: 1."<<i<<std::endl;
        candidate_sets.emplace_back();
        candidate_sets[i].reserve(qsize);
        for (int j = 0; j < qsize; ++j){
            candidate_sets[i].emplace_back();
            candidate_sets[i][j].reserve(max_degree);
        }
    }
    //candidate_sets.resize(qsize, std::vector<std::vector<ui>>(qsize));

    // std::cout<<"2"<<std::endl;
    // std::cout<<"qsize"<<qsize<<std::endl;
    candidate_set_flags.reserve(qsize);
    for (ui i = 0; i < qsize; ++i){
        candidate_set_flags.emplace_back();
        candidate_set_flags[i].resize(qsize, false);
    }

    // initial

    // std::cout<<"3"<<std::endl;
    key_vertex_set.resize(data_graph->getVerticesCount(), false);

    // std::cout<<"4"<<std::endl;

    // for (ui i = 0; i < qsize; ++i){ // ここは必要なのか？ ここが元凶？
    //     // std::cout<<"4.i: 1."<<i<<std::endl;
    //     ui query_vertex_id = order[i];
    //     candidate_sets[0][i].assign(candidates[query_vertex_id], candidates[query_vertex_id] + candidates_count[query_vertex_id]);
    // }
    ui query_vertex_id = order[0];
    candidate_sets[0][0].assign(candidates[query_vertex_id], candidates[query_vertex_id] + candidates_count[query_vertex_id]);
    ...
    for (ui i = 0; i < qsize; ++i){
        ui neighbor_count;
        const ui *neighbors = query_graph->getVertexNeighbors(i, neighbor_count);
...
std::cout << "Total time spent in computeCandidates (ms): " << manage_time / 1000000.0 << std::endl;
std::cout << "Total recursive calls: " << context.call_count << std::endl;
...
bool EvaluateQuery::computeCandidates(ui depth, const std::vector<ui>& current_match, MatCoContext& context){
    ...
    const ui *vf_neighbors = context.data_graph->getVertexNeighbors(vf, vf_neighbor_count);
    ...
    for (ui i = depth; i < context.query_graph->getVerticesCount(); ++i){
        ...
        if (context.query_adj_matrix[uf][ub]){
            ...
            if (context.candidate_sets[depth - 1][i].size() == 0) { // ここは、candidate_setsの初期値を直して０に変える
                ...
                //std::set_intersection(context.candidate_sets[depth-1][i].begin(),context.candidate_sets[depth-1][i].end(),vf_nbrs_vec.begin(),vf_nbrs_vec.end(),std::back_inserter(context.candidate_sets[depth][i]));
            }
            //std::cout<<"size of candidate_sets: "<<context.candidate_sets[depth][i].size()<<std::endl;
            if (context.candidate_sets[depth][i].empty()){
                cnt_1 = cnt_1 + 1;
                return false;
            } 
        } else {
...
bool EvaluateQuery::fullCoveragePrune(ui depth, const std::vector<ui>& current_match, MatCoContext& context){
    ...
    for (ui i = depth; i < qsize; i++){
        ...
        if (context.candidate_set_flags[depth - 1][i]){ //ここにも入れてない
            //context.call_count++;
            cnt_6 = cnt_6 + 1;
...
void EvaluateQuery::FlushFlag(ui flush_depth, MatCoContext& context){
    ui u = context.order[flush_depth];
    for (ui i = flush_depth + 1; i < context.query_graph->getVerticesCount(); i++){
        ui ub = context.order[i];
        if (context.query_adj_matrix[u][ub]) continue; // ここはこれでいいのか？
        else {
            context.candidate_set_flags[flush_depth][i] = true;
        }
    }
}
...
{
    //assumming that creating matrix can be done before we do not count the time.
    int qsiz = query_graph->getVerticesCount(); // qsiz = "query size"
    int ksize = 10000; // for top-k matching experiments
    int SolPos[ksize+1][qsiz];
    std::priority_queue<std::pair<int, int>, std::vector<std::pair<int, int>>, std::greater<>> pq; // pq {priority, data}
    int tempPos[qsiz];
    int tempPos1[qsiz];
    auto start = std::chrono::high_resolution_clock::now(); // start enumeration time
    pq.push({0,0});
    int heapcount = 0;
    enumResult s;
    std::vector<std::vector<int>> temp_sol(qsiz);
